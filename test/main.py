...
def get_currency(current):
    r = requests.get(
        f"https://free.currconv.com/api/v7/convert?q=RUB_{current},{current}_RUB&compact=ultra&apiKey=" + key).json()
    return current + ' в RUB ' + str(r[f'{current}_RUB']) + '\n' 'RUB в ' + current + ' ' + str(r[f'RUB_{current}'])
        ...
        if message:
            ...
            if message == "Пока":
                return send_message(chat_id,
                                    "До новых встреч, мой друг. Помни что курс валют меняется очень часто. Возвращайся скорее.")
            else:
                send_photo(chat_id)
